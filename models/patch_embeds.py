...
class MultiModalPatchEmbeds(nn.Module):
    ...
    def __init__(
        ...
        self.rgb = PatchEmbed(in_chans=3, embed_dim=embed_dim, patch_size=patch_size, img_size=img_size)
        self.ir = PatchEmbed(in_chans=1, embed_dim=embed_dim, patch_size=patch_size, img_size=img_size)        
        self.cpencil = PatchEmbed(in_chans=3, embed_dim=embed_dim, patch_size=patch_size, img_size=img_size)
        self.sketch = PatchEmbed(in_chans=1, embed_dim=embed_dim, patch_size=patch_size, img_size=img_size)
        
        self.modality_names = ['rgb', 'ir', 'cpencil', 'sketch']
    ...
    def forward(self, x: torch.Tensor, modality: str) -> torch.Tensor:
        ...
        Args:
            x: [B, C, H, W] 输入图像
            modality: 模态名称，'rgb'|'ir'|'cpencil'|'sketch'
        Returns:
            [B, num_patches, embed_dim] patch embeddings
        """
    ...
    def load_clip_weights(self, clip_patch_embed_weight: torch.Tensor, clip_bias: Optional[torch.Tensor] = None):
        ...
        for modality in self.modality_names:
            ...
            if modality != 'rgb':  # RGB保持原始权重，其他模态添加噪声
                with torch.no_grad():
...
if __name__ == "__main__":
    ...
    # 测试各模态
    rgb_img = torch.randn(2, 3, 224, 224)  # RGB图像
    ir_img = torch.randn(2, 1, 224, 224)   # 红外图像
    
    rgb_patches = patch_embeds(rgb_img, 'rgb')
    ir_patches = patch_embeds(ir_img, 'ir')
    
    print(f"RGB patches shape: {rgb_patches.shape}")  # [2, 196, 768]
    print(f"IR patches shape: {ir_patches.shape}")    # [2, 196, 768]
